...
A check on a job that is still running is worth the turn it costs: it names the account being spent, how long the run has gone, how long ago the delegate last wrote anything, how many seconds are left on each deadline before clauth kills it, and the newest line of the delegate's own answer. It also carries that account's 5h/7d windows where clauth has them cached; a third-party account reports its usage elsewhere, so the check reads `usage unknown` for one today. A run with no idle deadline says so rather than reading as a missing figure — pinning your own `--output-format` in `args` turns that leg off, because silence then carries no information.
