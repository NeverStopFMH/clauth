...
- A delegate is killed only once it has produced no output for `idle_secs` or has passed the `timeout_secs` wall clock, so a long run that keeps working is never cut off. A killed one still returns the text it had written, in `partial_result`.
- Pass a killed run's `session_id` back as `resume` to continue it rather than paying for the work again. clauth runs the resume in the workspace the session was recorded in. Shared delegates are always resumable; an isolated one needs auto-rescue on, since its transcript otherwise dies with its throwaway runtime.
- `delegate` accepts `model` (which model the run uses), `cwd`, `env`, `args`, `idle_secs` (default 300) and `timeout_secs` (default 3600, both max 3600), `resume`, `isolated` (a clean delegate with no operator memory, plugins, or hooks), `background`, plus `monitor` (a backgrounded job then reports elapsed time and the target's live usage on a `delegate_result` poll). clauth records the delegate's observed tokens/sec per model and flags it as degraded or recently rate-limited in `list_profiles` / `which`. That's the only throughput signal available, since subscription throttle is per-model and absent from the usage snapshot.
