...
| `clear long-lived token` | drops that token so the account's own OAuth login installs again, or signs Claude Code out when the account has only an api key behind it; the row's hint names which. Appears only when a token is stored, arms on the first press, clears on the second. Faint and inert when the account has no other login to fall back to |
