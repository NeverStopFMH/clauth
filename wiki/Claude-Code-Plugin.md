...
**Targets.** `profiles` takes one name or several. One name runs a single delegate, blocking by default; set `background` to get a job id back at once. Two or more fan out one delegate per named account, background-only, and spend one real usage window per account, returning one `job_id` per account and echoing the resolved target list. Duplicate names (case-insensitive), unknown names, a disabled member, a member with no inference auth (no usable api key and no `ANTHROPIC_AUTH_TOKEN`/`ANTHROPIC_API_KEY` in its `[env]`), an empty list, and a blocking fan-out are refused before any spawn.
