...
**Pane state.** In herdr, the pane's agent icon shows `working` for as long as any delegate runs, sync and background alike, and returns to `idle` when the last one ends. It reports only inside a herdr pane. On herdr 0.8.0 the report lands only where herdr's own Claude Code integration has not already claimed that pane's agent state: on a claimed pane herdr keeps its own state and the icon does not move. herdr drops a report for reasons of its own as well, so an unclaimed pane is no guarantee either. A stuck report is killed after two seconds, so it can never fail a delegate. It does cost that wait: a delegate sends one report when it starts and one when it ends, and the MCP server is single-threaded, so a stuck herdr holds up your other tool calls for up to two seconds at each end, four in total.
