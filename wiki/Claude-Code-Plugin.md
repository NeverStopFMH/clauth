...
| `monitor` | `job_ids` (optional list, capped at 256), `wait_secs` (0-60, default 0) | with `job_ids`: a backgrounded job's envelope, its running status, or `unknown` for an absent id, one result per id; with none: what moved in clauth's state, as soon as it moves | none |
